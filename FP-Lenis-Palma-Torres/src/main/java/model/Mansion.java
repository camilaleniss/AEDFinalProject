...
//		adjMatrix = new AdjMatrixGraph<>(true, true);
	 ...
	 * The mansion can have many exits, so it finds the minimum time with all the different options
	...
	public List<Room> shortestWayOut(String room) throws NotFoundException{
		...
		if (rr==null) throw new NotFoundException("The room does not exist");
...
//		return mintime;
	...
	public double getPathLength(List<Room> path) throws NotFoundException{
		...
		for (Room r : path) {
			...
			if(v == null) throw new NotFoundException("The room does not exist");
			total += v.getD();
		}
		return total;
	}
...
//		adjMatrix.addVertex(new Room(name, isExit));
	...
	public void createCorridor(String from, String to, double time) throws NotFoundException, CorridorAlreadyExistsException {
		Room fromRoom = searchRoom(from);
		Room toRoom = searchRoom(to);
